...
- **No network access.** HamMeter opens no listening port and makes no outgoing connection (Kuroukihime's update checker and icon downloader are not used).
...
This writes `artifacts\HamMeter-Setup-<version>.exe` and its SHA-256.
